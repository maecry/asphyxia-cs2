...
public:
	void SetShaderType(const char* szShaderName)
	{
		...
		using fnSetMaterialShaderType = void(CS_FASTCALL*)(CMaterialData*, MaterialKeyVar_t, const char*, int);
...
public:
	...
	CMaterial2** CreateMaterial(CMaterial2*** pOutMaterial, const char* szMaterialName, CMaterialData* pData)
	{
		return MEM::CallVFunc<CMaterial2**, 29U>(this, pOutMaterial, szMaterialName, pData, 0, 0, 0, 0, 0, 1);
	}

	void SetCreateDataByMaterial(CMaterial2*** pInMaterial, CMaterialData* pData)
	{
		MEM::CallVFunc<void, 37U>(this, pInMaterial, pData);
	}
};
